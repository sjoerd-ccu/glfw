                                ...
                                wl_fixed_t sy)
{
    ...
    if (window->cursorMode == GLFW_CURSOR_DISABLED)
    {
        ...
        _glfwInputError(GLFW_PLATFORM_ERROR,
                        "DRM: GLFW_CURSOR_DISABLED not supported");
        return;
    }
    else
    {
        window->drm.cursorPosX = wl_fixed_to_double(sx);
        window->drm.cursorPosY = wl_fixed_to_double(sy);
    }
                              ...
                              wl_fixed_t value)
{
    ...
    /* DRM scroll events are in pointer motion coordinate space (think
                                 ...
                                 uint32_t size)
{
    ...
    {
        _glfwInputError(GLFW_PLATFORM_ERROR,
                        "DRM: Failed to create XKB state");
...
int _glfwPlatformInit(void)
{
    ...
    _glfw.drm.gbm_device = gbm_create_device(_glfw.drm.fd);
    if (!_glfw.drm.gbm_device)
    {
        _glfwInputError(GLFW_PLATFORM_ERROR,
                        "DRM: Failed to create GBM device");
        return GLFW_FALSE;
    }
    ...
    {
        _glfwInputError(GLFW_PLATFORM_ERROR,
                        "DRM: Failed to initialize xkb context");
        return GLFW_FALSE;
    }
#endif
    ...
    if (_glfw.wl.pointer && _glfw.wl.shm)
    {
        ...
        {
            _glfwInputError(GLFW_PLATFORM_ERROR,
                            "DRM: Unable to load default cursor theme\n");
            return GLFW_FALSE;
        }
        ...
        {
            _glfwInputError(GLFW_PLATFORM_ERROR,
                            "DRM: Unable to load default left pointer\n");
            return GLFW_FALSE;
        }
...
void _glfwPlatformTerminate(void)
{
    ...
    if (_glfw.drm.gbm_device)
        gbm_device_destroy(_glfw.drm.gbm_device);
